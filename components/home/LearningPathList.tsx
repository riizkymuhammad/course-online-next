...
export default function LearningPathList({ items }: { items: LearningPathCard[] }) {
  return (
    <div className="mt-8 grid grid-cols-2 gap-3 sm:gap-5 lg:grid-cols-4">
      {items.map((item) => (
        <Link key={item.id} href={item.href} aria-label={`Buka learning path ${item.title}`} className="group block rounded-md outline-none focus-visible:ring-2 focus-visible:ring-brand-500 focus-visible:ring-offset-2">
